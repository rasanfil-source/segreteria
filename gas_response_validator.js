...
class ResponseValidator {
  ...
  _checkCapitalAfterComma(response, expectedLanguage = 'it') {
    ...
    const italianForbiddenCaps = [
      // Verbi
      'Siamo', 'Restiamo', 'Sono', '\u00C3\u02C6', "E'", 'Era', 'Sarà',
    ...
    const pattern = /,\s+([A-Z\u00C3\u20AC\u00C3\u02C6\u00C3\u2030\u00C3\u0152\u00C3\u2019\u00C3\u2122][a-zàèéìòù]*)/g;
    let match;
    const violations = [];
    ...
    for (const [timeSlot, greetings] of Object.entries(patterns)) {
      ...
      for (const greeting of greetings) {
        ...
        if (match) {
          ...
          console.log(`   🔍\u201E Saluto "${originalGreeting}" \u00E2\u2020\u2019 "${replacement}" (ore ${currentHour}:00)`);
          return fixedText;
        }
      }
    }
  ...
  _ottimizzaCapitalAfterComma(text, language) {
    ...
    targets.forEach(word => {
      ...
      result = result.replace(regex, (fullMatch, p1, offset) => {
        const firstLetter = p1.charAt(0);
        const rest = p1.slice(1);

        // Prima di preservare la maiuscola per nome doppio, forziamo i casi vietati
        const forbiddenWords = ['Siamo', 'Sono', 'Ho', 'Hai', 'Abbiamo', 'Il', 'La', 'I', 'Le', 'Un', 'Una', 'In', 'Con', 'Per', 'Su'];
        if (forbiddenWords.includes(p1)) {
...
class SemanticValidator {
  ...
  validateHallucinations(response, knowledgeBase, regexResult, emailContent) {
    if (!this.shouldRun(regexResult.score) && regexResult.errors.length === 0) {
      console.log('   \u00E2\u0161\u00A1 Semantic hallucination ✅ skippato (confidence alta)');
